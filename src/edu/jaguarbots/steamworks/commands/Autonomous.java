	...
	public Autonomous(final Robot.Recording recording, final Robot.Position position, final Robot.MiddlePosition middlePosition, final Robot.Alliance alliance) {
		...
		switch (recording) {
			case RecordNewAuto:
				...
				String fileName = "/home/lvuser/" + Robot.positionChooser.getSelected().toString() + ((Robot.positionChooser.getSelected() == Robot.Position.Middle) ? Robot.middlePositionChooser.getSelected().toString() : "") + Robot.allianceChooser.getSelected().toString();
					...
					try {
						...
						for (int i = 0; i < commands.size(); i++) {
							if (commands.get(i) == "straight")
								addSequential(new EncoderDrive(encoderTicks.get(i)));
							if (commands.get(i) == "turn")
								addSequential(new EncoderTurn(encoderTicks.get(i)));
						}
